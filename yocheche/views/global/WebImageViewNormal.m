...
-(void)setWebImageViewWithURL:(NSURL *)url
{
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:[NSString stringWithFormat:@"%@/img/%@",[NSSearchPathForDirectoriesInDomains( NSCachesDirectory, NSUserDomainMask, YES ) objectAtIndex: 0],strUrl]])
    {
        NSData *myData= [NSData dataWithContentsOfFile:[NSString stringWithFormat:@"%@/img/%@",[NSSearchPathForDirectoriesInDomains( NSCachesDirectory, NSUserDomainMask, YES ) objectAtIndex: 0],strUrl]];
        if (myData.length>0)
        {
...
-(void)setWebPImageViewWithURL:(NSURL *)url
{
    ...
    if ([[NSFileManager defaultManager] fileExistsAtPath:[NSString stringWithFormat:@"%@/img/%@",[NSSearchPathForDirectoriesInDomains( NSCachesDirectory, NSUserDomainMask, YES ) objectAtIndex: 0],strUrl]])
    {
        ...
        if (myData.length>0)
        {
            self.image = [UIImage imageWithWebPData:myData];
        }
    ...
    else
    {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_BACKGROUND, 0), ^{
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                // Update the UI
                if (imageData.length>0) {
                    self.image = [UIImage imageWithWebPData:imageData];
                    if ([imageData writeToFile:[self getImagePath:strUrl] atomically:YES])
                    {
                        NSLog(@"saved");
                    }
