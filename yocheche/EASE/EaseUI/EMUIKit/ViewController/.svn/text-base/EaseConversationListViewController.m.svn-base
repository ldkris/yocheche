...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (self.dataArray.count>0)
    {
        ...
        //yocheche
        EaseConversationModel *modelConversation=[self.dataArray objectAtIndex:indexPath.row];
//        cell.avatarView.layer.masksToBounds=YES;
//        [cell.avatarView.layer setCornerRadius:PARENT_WIDTH(cell.avatarView)/2.0];
        cell.titleLabel.text=modelConversation.title;
        [cell.avatarView setWebImageViewWithURL:[NSURL URLWithString:modelConversation.avatarURLPath]];
        //yochecheend
        return cell;
    }
    else
    {
        NoChatDataCell *cell=[tableView dequeueReusableCellWithIdentifier:@"NoChatDataCell"];
        if(cell==nil)
        {
            cell=[[[NSBundle mainBundle] loadNibNamed:@"NoChatDataCell" owner:nil options:nil] objectAtIndex:0];
        }
        return cell;
    }
}
...
- (void)tableViewDidTriggerHeaderRefresh
{
    ...
    for (EMConversation *converstion in sorted)
    {
        ...
        if (model)
        {
            ...
            unreadCountAll=unreadCountAll+[model.conversation unreadMessagesCount];
        }
    }

    AppDelegate *appdelegate=(AppDelegate*)[[UIApplication sharedApplication] delegate];
    [appdelegate setTabBarUnreadMessageCount:unreadCountAll];
