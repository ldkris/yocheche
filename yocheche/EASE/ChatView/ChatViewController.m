...
- (void)viewDidLoad {
    ...
    [self.tableView setBackgroundColor:YCC_GrayBG];
...
- (void)_setupBarButtonItem
{
    ...
    //yocheche
    [backButton setImageEdgeInsets:UIEdgeInsetsMake(12, 0, 12, 32)];
    //yocheche end
    ...
    if (self.conversation.conversationType == eConversationTypeChat) {
        ...
        [clearButton setImageEdgeInsets:UIEdgeInsetsMake((44-PARENT_HEIGHT(imageview))/2.0, 44-PARENT_WIDTH(imageview), (44-PARENT_HEIGHT(imageview))/2.0, 0)];
        //yocheche end
...
//    [self.navigationController pushViewController:userprofile animated:YES];
    ...
    vc.preData=[NSDictionary dictionaryWithObject:messageModel.isSender?self.accountMy:self.accountOther forKey:@"account"];
...
//    }
    ...
    if ([message.from isEqualToString:self.idMy])
    {
        model.avatarURLPath=self.avatarURl;
        model.nickname=self.nameMy;
    }
    else
    {
        model.avatarURLPath=self.avatarURlOther;
        model.nickname=self.nameOther;
    }
    //end
    return model;
}
