...
@interface MyCoachListVC ()

@end

@implementation MyCoachListVC
...
{
    [MobClick beginLogPageView:@"MyCoachListVC"];
}
-(void)viewWillDisappear:(BOOL)animated
{
    [MobClick endLogPageView:@"MyCoachListVC"];
}
-(void)updateData
{
    [self showLoadingWithBG];
    Http *req=[[Http alloc] init];
    req.socialMethord=@"user/getMyCoach.yo";
...
{
    MyCoachCell *cell=[tableView dequeueReusableCellWithIdentifier:@"MyCoachCell"];
    if (cell==nil)
    {
        cell=[[[NSBundle mainBundle] loadNibNamed:@"MyCoachCell" owner:nil options:nil] objectAtIndex:0];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    vc.preData=[NSDictionary dictionaryWithObject:[[self.m_aryData objectAtIndex:indexPath.row] objectForKey:@"coachid"] forKey:@"coachId"];
