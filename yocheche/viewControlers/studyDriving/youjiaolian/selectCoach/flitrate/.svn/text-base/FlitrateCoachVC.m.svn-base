...
@interface FlitrateCoachVC ()

@end

@implementation FlitrateCoachVC
...
- (void)viewDidLoad {
    ...
    self.m_arySort=[NSMutableArray arrayWithObjects:@"价格从低到高",@"价格从高到低",@"距离从近到远",nil];
    self.m_aryLicenseType=[NSMutableArray arrayWithObjects:@"不限",@"C1",@"C2", nil];
    [self addTableView];
...
{
    [MobClick beginLogPageView:@"FiltrateCoachVC"];
}
- (void)viewWillDisappear:(BOOL)animated
{
    [MobClick endLogPageView:@"FiltrateCoachVC"];
}
...
{
    FlitrateCoachCell *cell=[[[NSBundle mainBundle] loadNibNamed:@"FlitrateCoachCell" owner:nil options:nil] objectAtIndex:0];
    cell.delegate=self;
    [cell updateViews];
    return cell;
}
...
-(void)creatPickerView:(NSInteger)index
{
    ...
    if (index==0)
    {
         self.m_currentSelectArray= self.m_aryLicenseType;
        self.currentSelectKey=@"drivetype";
    }
    else if (index==7)
    {
          self.m_currentSelectArray=self.m_arySort;
        self.currentSelectKey=@"sort";
    }
...
-(void)selectDone
{
    ...
    else if (self.pickerView.tag==7)
    {
        [self.m_filtrateData setObject:[NSString stringWithFormat:@"%d",index+2] forKey:self.currentSelectKey];
    }
...
-(void)clearParams
{
    self.m_filtrateData=[NSMutableDictionary dictionaryWithObjects:@[[NSString stringWithFormat:@"%f",self.m_currentLocation.latitude],[NSString stringWithFormat:@"%f",self.m_currentLocation.longitude],@"2",@"",@""] forKeys:@[@"lat",@"lng",@"sort",@"drivetype",@"dsname"]];
    [self.m_tableView reloadData];
}
-(void)showFixLocationVC
{
    FixLocationVC *vc=[[FixLocationVC alloc] init];
    vc.delegateFlitrate=self;
...
-(IBAction)filtrateBtnPressed:(id)sender
{
    ...
    self.delegate.m_coachListView.pageIndex=1;
    
    self.delegate.m_currentAddress=self.m_currentAddress;
    self.delegate.m_flitrateDataCoach=[NSMutableDictionary dictionaryWithDictionary:self.m_filtrateData];
